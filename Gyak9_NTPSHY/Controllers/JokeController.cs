using Gyak9_NTPSHY.JokeModels;
using Microsoft.AspNetCore.Mvc;

// For more information on enabling Web API for empty projects, visit https://go.microsoft.com/fwlink/?LinkID=397860

namespace Gyak9_NTPSHY.Controllers
{
    [Route("api/jokes")]
    [ApiController]
    public class JokeController : ControllerBase
    {
        ...
        {
            FunnyDatabaseContext context = new FunnyDatabaseContext();
            var lekerdezes = from x in context.Jokes
                             where x.JokeSk == id
                             select x;
            return Ok(lekerdezes.FirstOrDefault());
        }
        ...
        {
            FunnyDatabaseContext context = new FunnyDatabaseContext();
            var törlés = from x in context.Jokes
                         where x.JokeSk == id
                         select x;
            context.Remove(törlés.FirstOrDefault());
            context.SaveChanges();
        }
    }
}
